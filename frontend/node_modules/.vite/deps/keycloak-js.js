...
//#endregion
//#region node_modules/keycloak-js/dist/keycloak.mjs
...
function Keycloak(config) {
	if (!(this instanceof Keycloak)) throw new Error("The 'Keycloak' constructor must be invoked with 'new'.");
	var kc = this;
	var adapter;
	var refreshQueue = [];
	var callbackStorage;
	...
	kc.init = function(initOptions) {
		...
		if (initOptions && [
			...
			"cordova",
			"cordova-native"
		].indexOf(initOptions.adapter) > -1) adapter = loadAdapter(initOptions.adapter);
		...
		if (initOptions) {
			...
			if (initOptions.responseMode) if (initOptions.responseMode === "query" || initOptions.responseMode === "fragment") kc.responseMode = initOptions.responseMode;
			...
			if (initOptions.flow) {
				switch (initOptions.flow) {
					...
					case "hybrid":
						...
						break;
					default: throw "Invalid value for flow";
				}
			...
			else kc.messageReceiveTimeout = 1e4;
		}
		...
		function onLoad() {
			...
			var checkSsoSilently = function() {
				...
				var messageCallback = function(event) {
					...
					processCallback(parseCallback(event.data), initPromise);
			...
			switch (initOptions.onLoad) {
				...
				case "login-required":
					doLogin(true);
					break;
				default: throw "Invalid value for onLoad";
			}
		}
		function processInit() {
			...
			else if (initOptions) if (initOptions.token && initOptions.refreshToken) {
	...
	function generatePkceChallenge(pkceMethod, codeVerifier) {
		...
		return bytesToBase64(new Uint8Array(import_sha256.default.arrayBuffer(codeVerifier))).replace(/\+/g, "-").replace(/\//g, "_").replace(/\=/g, "");
	}
	...
	kc.createLoginUrl = function(options) {
		...
		else if (scope.indexOf("openid") === -1) scope = "openid " + scope;
	...
	kc.createLogoutUrl = function(options) {
		if ((options?.logoutMethod ?? kc.logoutMethod) === "POST") return kc.endpoints.logout();
	...
	kc.updateToken = function(minValidity) {
		...
		if (loginIframe.enable) checkLoginIframe().then(function() {
			exec();
		}).catch(function(error) {
			promise.setError(error);
		});
	...
	function getRealmUrl() {
		if (typeof kc.authServerUrl !== "undefined") if (kc.authServerUrl.charAt(kc.authServerUrl.length - 1) == "/") return kc.authServerUrl + "realms/" + encodeURIComponent(kc.realm);
	...
	function loadConfig(url) {
		...
		if (configUrl) {
			...
			req.setRequestHeader("Accept", "application/json");
			req.onreadystatechange = function() {
				if (req.readyState == 4) if (req.status == 200 || fileLoaded(req)) {
			...
			if (!oidcProvider) {
				...
				req.onreadystatechange = function() {
					if (req.readyState == 4) if (req.status == 200 || fileLoaded(req)) {
						setupOidcEndoints(JSON.parse(req.responseText));
						promise.setSuccess();
					} else promise.setError();
				};
				req.send();
	...
	function setToken(token, refreshToken, idToken, timeLocal) {
		...
		if (token) {
			...
			if (timeLocal) kc.timeSkew = Math.floor(timeLocal / 1e3) - kc.tokenParsed.iat;
			if (kc.timeSkew != null) {
	...
	function loadAdapter(type) {
		if (!type || type == "default") return {
			...
			logout: async function(options) {
				if ((options?.logoutMethod ?? kc.logoutMethod) === "GET") {
		...
		if (type == "cordova") {
			...
			return {
				login: function(options) {
					...
					var ref = cordovaOpenWindowWrapper(kc.createLoginUrl(options), "_blank", cordovaOptions);
					var completed = false;
					var closed = false;
					...
					ref.addEventListener("loadstart", function(event) {
						if (event.url.indexOf(getCordovaRedirectUri()) == 0) {
							processCallback(parseCallback(event.url), promise);
							closeBrowser();
							completed = true;
						}
					});
					...
					ref.addEventListener("exit", function(event) {
						if (!closed) promise.setError({ reason: "closed_by_user" });
					});
					return promise.promise;
				},
				logout: function(options) {
					var promise = createPromise();
					var ref = cordovaOpenWindowWrapper(kc.createLogoutUrl(options), "_blank", "location=no,hidden=yes,clearcache=yes");
					var error;
					ref.addEventListener("loadstart", function(event) {
				...
				register: function(options) {
					var promise = createPromise();
					var ref = cordovaOpenWindowWrapper(kc.createRegisterUrl(), "_blank", createCordovaOptions(options));
					ref.addEventListener("loadstart", function(event) {
						if (event.url.indexOf(getCordovaRedirectUri()) == 0) {
							ref.close();
							processCallback(parseCallback(event.url), promise);
	...
	var LocalStorage = function() {
		...
		cs.add = function(state) {
			...
			state.expires = (/* @__PURE__ */ new Date()).getTime() + 3600 * 1e3;
