...
public class AesEncryptionConverter implements AttributeConverter<String, String> {
    ...
    public AesEncryptionConverter(@Value("${encryption.secret-key}") String hexKey) {
        // Decode hex string to bytes (64 hex chars -> 32 bytes for AES-256)
        byte[] keyBytes = decodeHex(hexKey);
        if (keyBytes.length != 32) {
            throw new IllegalArgumentException(
                "AES key must be exactly 32 bytes (64 hex characters); got " + keyBytes.length + " bytes from " + hexKey.length() + " hex chars");
        }
     ...
     * @param hex hex string (e.g., "0123456789abcdef...")
