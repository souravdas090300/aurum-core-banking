...
import com.aurum.core_banking.infrastructure.security.KeycloakRoleConverter;
...
class KeycloakRoleConverterTest {
    ...
    void realmRoles_mappedToSpringAuthorities() {
        Jwt jwt = buildJwt(Map.of("roles", List.of("banking-user", "loan-officer")));

        Collection<GrantedAuthority> authorities = converter.convert(jwt);

        assertThat(authorities).extracting(GrantedAuthority::getAuthority)
                .containsExactlyInAnyOrder("ROLE_BANKING_USER", "ROLE_LOAN_OFFICER");
    }
